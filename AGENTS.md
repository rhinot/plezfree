...
For agents or environments that prefer `npm`, the following commands are available as wrappers around the Flutter commands:

*   **Setup**: `npm install` (or `npm ci`) will run `flutter pub get` via the `postinstall` script.
*   **Test**: `npm test` runs `flutter test`.
*   **Lint**: `npm run lint` runs `flutter analyze`.
*   **Build**: `npm run build` runs the code generation (`dart run build_runner build`).
